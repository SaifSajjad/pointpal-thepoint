...
export function executePointPalTool(
  ...
  const positivePreferences = requestedPreferences.filter((preference) =>
    !/cheap|cheapest|afford|budget|inexpensive|economical|value|low[ -]?price|lowest[ -]?price|sasti|sasta|saste|popular|best|achi|acha|recommend|suggest|batao|bata|kro|karo|less sweet|low sweet|kam sweet/i.test(preference),
  );
